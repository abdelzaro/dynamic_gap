...
{   
                                ...
                                const ros::Time & tCurrentFilterUpdate) 
    {
        ...
        try
        {
            for (int i = 0; i < 2*gaps.size(); i++) 
            {
                ...
                identifyDynamicEndPoints(i, gaps, intermediateRbtVels, intermediateRbtAccs, tCurrentFilterUpdate);
  ...
  void Planner::identifyDynamicEndPoints(const int & idx, 
                                std::vector<dynamic_gap::Gap *> & gaps, 
                                const std::vector<geometry_msgs::TwistStamped> & intermediateRbtVels,
                                const std::vector<geometry_msgs::TwistStamped> & intermediateRbtAccs,
                                const ros::Time & tCurrentFilterUpdate) 
    {
		try
        {
            ...
            // TODO comment this if statement out because it's unused
            if (haveLastOdomMsg_)
            {
                rbtVelOdomFrame.header = lastRbtOdomMsg_.header; // storing last robot velocity to try to find relative velocity
                rbtVelOdomFrame.vector = lastRbtOdomMsg_.twist.twist.linear;
                // ROS_ERROR_STREAM_NAMED("identifyDynamicEndPoints", "inside indentifyDynamicEndPoints rbtVelOdomFrame.vector");
                // ROS_ERROR_STREAM_NAMED("identifyDynamicEndPoints", rbtVelOdomFrame);

            }
            else
            {
                // If we haven't received any odom yet, just default to zero
                // ROS_ERROR_STREAM_NAMED("identifyDynamicEndPoints", "we haven't received any odom yet, just default to zero");

                rbtVelOdomFrame.header.stamp = ros::Time::now();
                rbtVelOdomFrame.vector.x = 0.0;
                rbtVelOdomFrame.vector.y = 0.0;
                rbtVelOdomFrame.vector.z = 0.0;
            }
            ...
            if (idx % 2 == 0) 
            {
                if (gap->leftGapPtModel_)
                {
                // std::string isStaticStr = gap->getIsStatic() ? "true" : "false"; // just for debugging
            
                // ROS_ERROR_STREAM_NAMED("identifyDynamicEndPoints", "left gap testing: " << isStaticStr);
                gap->leftGapPtModel_->isolateGapDynamics();
                // Eigen::Vector4f endPointState = gap->leftGapPtModel_->getGapState();

                // ROS_ERROR_STREAM_NAMED("identifyDynamicEndPoints", "endPointState");

                // ROS_ERROR_STREAM_NAMED("identifyDynamicEndPoints", endPointState);

                // ROS_ERROR_STREAM_NAMED("identifyDynamicEndPoints", gap->leftGapPtModel_->);

                // Eigen::Vector4f endPointState = gap->leftGapPtModel_->getGapVelocity();
                if (idx == 2)
                {
                Eigen::Vector2f gapVelocity = gap->leftGapPtModel_->getGapVelocity();
                Eigen::Vector2f RbtVel = gap->leftGapPtModel_->getRbtVel();
                
                Eigen::Vector2f speed = RbtVel + gapVelocity; 
                
                // ROS_ERROR_STREAM_NAMED("identifyDynamicEndPoints", "getGapVelocity output only for idx == 2!!!!");

                // ROS_ERROR_STREAM_NAMED("identifyDynamicEndPoints", gap->leftGapPtModel_->getGapVelocity());

                // ROS_ERROR_STREAM_NAMED("identifyDynamicEndPoints", RbtGapPtRelVel);

                // ROS_ERROR_STREAM_NAMED("identifyDynamicEndPoints", "speed: ");
                // ROS_ERROR_STREAM_NAMED("identifyDynamicEndPoints", "planner.cpp containing RbtVel + gapVelocity for idx2: ");
                // ROS_ERROR_STREAM_NAMED("identifyDynamicEndPoints", speed);
                ...
                {
                ROS_WARN_STREAM_NAMED("identifyDynamicEndPoints", "left model is null (in identifyDynamicEndPoints())");
                }
            } else 
            {
                if (gap->rightGapPtModel_) // abdel: make sure you run isolateGapDynamics for rightGapPtModel
                {
                ROS_INFO_STREAM_NAMED("identifyDynamicEndPoints", " ");
                    
                } else
                {                
                    ROS_WARN_STREAM_NAMED("identifyDynamicEndPoints", "right model is null (in identifyDynamicEndPoints())");
                }
            } 
        } catch (...) 
        {
            ROS_WARN_STREAM_NAMED("identifyDynamicEndPoints", "updateModel failed (in identifyDynamicEndPoints())");
        }        
    }
...
void Planner::jointPoseAccCB(const nav_msgs::Odometry::ConstPtr & rbtOdomMsg, 
                                 const geometry_msgs::TwistStamped::ConstPtr & rbtAccelMsg)
    {
        ...
        if (rbtOdomMsg->header.frame_id != cfg_.odom_frame_id)
            ROS_ERROR_STREAM("Odom msg header frame " << rbtOdomMsg->header.frame_id << " not same as cfg odom frame:" << cfg_.odom_frame_id);

        if (rbtOdomMsg->child_frame_id != cfg_.robot_frame_id)
            ROS_ERROR_STREAM("Odom msg child frame " << rbtOdomMsg->child_frame_id << " not same as cfg rbt frame:" << cfg_.robot_frame_id);
        
        if (rbtAccelMsg->header.frame_id != cfg_.robot_frame_id)
            ROS_ERROR_STREAM("Accel msg header frame " << rbtAccelMsg->header.frame_id << " not same as cfg rbt frame:" << cfg_.robot_frame_id);
        ...
        try
        {
            ...
            // ROS_ERROR_STREAM_NAMED("rbtVelOdomFrame", "rbtVelOdomFrame");
            // ROS_ERROR_STREAM_NAMED("rbtVelOdomFrame", rbtVelOdomFrame.vector);
            ...
            // ROS_ERROR_STREAM_NAMED("rbtVelRbtFrame.vector", "rbtVelRbtFrame.vector");
            // ROS_ERROR_STREAM_NAMED("rbtVelRbtFrame.vector", rbtVelRbtFrame.vector);
            ...
            lastRbtOdomMsg_ = *rbtOdomMsg;
            haveLastOdomMsg_ = true;   
        ...
        try 
        {
            for (size_t i = 0; i < gaps.size(); i++) 
            {
                ...
                if (runGoToGoal) 
                {
                    ...
                    trajEvaluator_->evaluateTrajectory(goToGoalTraj, goToGoalPoseCosts, goToGoalTerminalPoseCost, futureScans, gaps.at(i), i);
                ...
                trajEvaluator_->evaluateTrajectory(pursuitGuidanceTraj, pursuitGuidancePoseCosts, pursuitGuidanceTerminalPoseCost, futureScans, gaps.at(i), i);
            ...
            trajEvaluator_->evaluateTrajectory(idlingTrajectory, idlingPoseCosts, idlingTerminalPoseCost, futureScans, nullptr); // nullptr passed in because gap information is needed for social considation which isn't relevant when idling
                                                            ...
                                                            const std::vector<sensor_msgs::LaserScan> & futureScans) // bool isIncomingGapAssociated,
    {
        ...
        {            
            ...
            trajEvaluator_->evaluateTrajectory(incomingTraj, incomingPathPoseCosts, incomingPathTerminalPoseCost, futureScans, nullptr);
            ...
            trajEvaluator_->evaluateTrajectory(reducedCurrentTraj, currentPathPoseCosts, currentPathTerminalPoseCost, futureScans, nullptr);
